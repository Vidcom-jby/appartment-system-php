...
if (!$_POST) {
	...
	if(empty($errors)) {
		...
		if (!empty($post_data['id'])) {
			...
			$old_id=$post_data['id'];
			$conn_query = "UPDATE user_table SET surname='$surname',other_name='$other_name',email='$email',password='$password',user_group='PERSONNEL', phone_number=$phone_number, updated=now() WHERE id='$old_id'";
		} else {
			$conn_query = "INSERT INTO user_table SET surname='$surname',other_name='$other_name',email='$email',password='$password',user_group='PERSONNEL', phone_number=$phone_number, created=now(), id='$new_unique_hash'";
		}
