...
$stmt = $conn->prepare("SELECT * FROM course_table WHERE level=:level AND department=:department");
$stmt->execute(array(':level'=>$_SESSION['user']['level'],':department'=>$_SESSION['user']['department']));
$course_details=array();
while( $data=$stmt->fetch(PDO::FETCH_ASSOC)){
	if(!empty($data['unit']) && intval($data['unit'])>2){
		$data['cost']=2500;
	}else{
		$data['cost']=2000;
	}
...
if(empty($course_details)) {
	$errors['no_course_available'] = "<p>Sorry, Registrable Courses are not available for your Level or Department  </p>";
}
...
if(empty($errors['no_course_available'])) {
	if (!$_POST) {
		...
		if (empty($errors)) {
			...
			if (!empty($selected_courses)) {
				...
				if (empty($errors)) {
					$stmt = $conn->prepare("SELECT * FROM course_reg_table WHERE user_id=:user_id");
					...
					if (!empty($row)) {
						...
						$user_query = "UPDATE course_reg_table SET user_id='$id', department='$department', level='$level', amount=$total_amount, matric_number='$matric_number', registered_courses='$update_courses' WHERE id='" . $row['id'] . "'";
						if ($conn->query($user_query)) {
							...
							$user_query = "UPDATE user_table SET registered_courses='$new_registered_courses',total_expenses='$total_expenses',wallet_balance='$wallet_balance' WHERE id='$id'";
							if ($conn->query($user_query)) {
								$_SESSION['success'] = "<p>Your Course Registration was Successful. Click <span><a href='students/register_exams.php'>here</a> to register for Exams</span></p>";
								$test_course = array();
								$check_all = null;
								...
								$error['failure'] = "<h4>Sorry, Your Course Registration could not be completed. please try again later!</h4>";
							}
						...
						$wallet_balance = ($wallet_balance < 0) ? 0 : $wallet_balance;
						$total_expenses = !empty($total_expenses)?($total_expenses + $total_amount):$total_amount;
						$user_query = "INSERT INTO course_reg_table SET user_id='$id', department='$department', level='$level', amount=$total_amount, matric_number='$matric_number', id='$new_unique_hash', registered_courses='$selected_courses' ";
						if ($conn->query($user_query)) {
							$user_query = "UPDATE user_table SET registered_courses='$registered_courses',total_expenses='$total_amount',wallet_balance=$wallet_balance WHERE id ='$id'";
							if ($conn->query($user_query)) {
								$_SESSION['success'] = "<p>Your Course Registration was Successful. Click <span><a href='students/register_exams.php'>here</a> to register for Exams</span></p>";
								$test_course = array();
								$check_all = null;
								...
								$errors['failure'] = "<h4>Sorry, Your Course Registration could not be completed. please try again later!</h4>";
							}
						} else {
							$errors['failure'] = "<h4>Sorry, Your Course Registration could not be completed. please try again later!</h4>";
						}
	...
	$_SESSION['registered_courses']=empty($_SESSION['registered_courses'])?(!empty($_SESSION['user']['registered_courses'])?$_SESSION['user']['registered_courses']:''):$_SESSION['registered_courses'];
