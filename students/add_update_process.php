...
if (!$_POST) {
	...
	if(empty($errors)) {
		...
		if (!empty($post_data['id'])) {
			...
			$old_id=$post_data['id'];
			$conn_query = "UPDATE user_table SET department='$department',gender='$gender', image='$image', level='$level', matric_number='$matric_number',surname='$surname',other_name='$other_name',email='$email',study_centre='$study_centre',password='$password',state='$state',lga='$lga', user_group='STUDENT', phone_number=$phone_number, updated=now() WHERE id='$old_id'";
		} else {
			$conn_query = "INSERT INTO user_table SET department='$department', gender='$gender', image='$image', level='$level', matric_number='$matric_number',surname='$surname',other_name='$other_name',email='$email',study_centre='$study_centre',password='$password',state='$state',lga='$lga', created=now(), user_group='STUDENT', phone_number=$phone_number, id='$new_unique_hash'";
		}
		if ($conn->query($conn_query)){
			...
			if(!empty($_SESSION['user']) && !empty($_SESSION['user']['password']) && !empty($password) && $_SESSION['user']['password'] !==$password){
			    header('Location: ../logout.php');
			    exit();
			}else{
				if(!empty($_SESSION['user'])){
					header('Location: ./add_update.php?id='.$_SESSION['user']['id']);
				}else{
